 ...
 *
 * Only the GENERAL institution list is cached (memory + on-device disk) so
 * repeat lookups skip that one slow fetch. Verification results are never
 * persisted: every completed account number is re-verified on request,
 * because accounts open and close in the real world.
 *
 * The OkHttp dispatcher is widened so every resolve probe flies at once
 * instead of being serialized five at a time.
 */
